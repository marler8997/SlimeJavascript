...
function setPatheticWhiteSlime(ai) {
  ...
  ai.moveLogic = function(me,enemy) {
    ...
    var something;
    if(me.y > 0 && this.meToEnemyWall < 1150) {
      something = 0;
    } else {
      something = 50 + Math.trunc(20 * Math.random());
    }
    //console.log('xWhenBallBelow125: ' + xWhenBallBelow125 + ', something: ' + something);
    if(this.state != -1 || (this.ballVXToEnemyWall == 0 && this.ballToEnemyWall == 1600)) {
      if (this.state == -1) {
        if(this.enemyToTheirWall > 500) {
	  this.state = 0;
	} else {
	  this.state = 1;
	}
    ...
    if (xWhenBallBelow125 < 1000)
    {
      if (Math.abs(this.meToEnemyWall - 1332) < 40) {
        this.stopMovement();
      } else if (this.meToEnemyWall > 1332) {
    ...
    if (Math.abs(this.meToEnemyWall - xWhenBallBelow125) < something)
    {
      ...
      if (
	  (this.meToEnemyWall >= 1800 && this.ballToEnemyWall > 1660) ||
	  (this.meToEnemyWall <= 1160 && this.ballToEnemyWall < 1060 && Math.abs(this.ballToEnemyWall - this.meToEnemyWall) < 200)
	  ) {
        ...
        this.randomJump85Percent();
                 ...
                 (this.ballToEnemyWall - this.meToEnemyWall < 60) &&
		 (this.ballToEnemyWall != this.meToEnemyWall)) {
        this.randomJump85Percent();
      } else if ((Math.abs(this.ballToEnemyWall - this.meToEnemyWall) < 300) &&
    ...
    if (this.state == -1) {
	  ...
	  this.stopMovement();
        ...
        if (this.meToEnemyWall < 1150) {
          return;
        }
        if (this.meToEnemyWall > 1800)
        {
